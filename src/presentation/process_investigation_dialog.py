...
class ProcessInvestigationDialog(QDialog):
    ...
    def create_buttons(self, parent_layout: QVBoxLayout) -> None:
        ...
        self.copy_markdown_button = QPushButton("Copy Processes")
        ...
        self.copy_env_button = QPushButton("Copy Environment")
    ...
    def copy_process_environment(self) -> None:
        """Copy the environment variables of the selected process to clipboard."""
        if not self.selected_pid:
            QMessageBox.warning(self, "No Process Selected", "Please select a process first.")
            return

        try:
            # Get process environment
            report = self.process_service.get_process_environment_report(self.selected_pid)

            if not report or not report.all_variables:
                QMessageBox.warning(
                    self, "No Environment Data",
                    f"Could not retrieve environment variables for process {self.selected_pid}."
                )
                return

            # Generate environment variables export
            env_content = self._generate_process_environment_text(report)

            # Copy to clipboard
            from PyQt6.QtWidgets import QApplication
            clipboard = QApplication.clipboard()
            clipboard.setText(env_content)

            QMessageBox.information(
                self, "Environment Variables Copied",
                f"Environment variables ({len(report.all_variables)} variables) "
                f"from process {self.selected_pid} have been copied to clipboard."
            )
    ...
    def _generate_process_environment_text(self, report) -> str:
        ...
        lines = []
        lines.append(f"# Environment Variables - Process {report.process.pid}")
        lines.append("")
        lines.append(f"**Process:** {report.process.name}")
        lines.append(f"**Command:** {report.process.command_line}")
        lines.append(f"**User:** {report.process.username}")
        lines.append(f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append(f"**Total Variables:** {len(report.all_variables)}")
        lines.append("")

        # Environment variables in simple format (for easy copying)
        ...
        # Also include shell export format
        lines.append("## Shell Export Format")
        lines.append("")
        lines.append("```bash")
        for name, value in sorted(report.all_variables.items()):
            # For shell export, we need to handle special characters
